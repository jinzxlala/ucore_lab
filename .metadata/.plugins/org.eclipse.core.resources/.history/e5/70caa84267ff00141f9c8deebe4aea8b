...
alloc_proc(void) {
    ...
    //LAB8:EXERCISE2 YOUR CODE HINT:need add some code to init fs in proc_struct, ...
    	...
    	proc->lab6_stride = 0;//current->lab6_stride;
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    */
    ...
    //    5. insert proc_struct into hash_list && proc_list
    bool _flag;
    local_intr_save(_flag);
    {
    	proc->pid = get_pid();
    	hash_proc(proc);
    	set_links(proc);
    }
    local_intr_restore(_flag);
    //    6. call wakup_proc to make the new child process RUNNABLE
    wakeup_proc(proc);
    //    7. set ret vaule using child proc's pid
    ret = proc->pid;
...
load_icode(int fd, int argc, char **kargv) {
    ...
    uint32_t current_size = 0, k;
    uintptr_t newstacktop;
    for(k = 0; k < argc; k++){
    	current_size += strnlen(kargv[k], EXEC_MAX_ARG_LEN + 1) + 1;
    }
    newstacktop = USTACKTOP - (current_size/sizeof(long) + 1) * sizeof(long);
    current_size = 0;
    char **uargv = (char **)(newstacktop - argc * sizeof(char*));
    for(k = 0; k < argc; k++){
    	uargv[k] = strcpy((char*)(newstacktop + current_size), kargv[k]);
    	current_size += strnlen(kargv[k], EXEC_MAX_ARG_LEN + 1) + 1;
    }
