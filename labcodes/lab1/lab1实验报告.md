...
【练习一】
1.操作系统镜像文件 ucore.img 是如何一步一步生成的?(Makefile中每一条相关命令和命令参数的含义 需要比较详细地解释,以及说明命令导致的结果)
```
根据lab1/Makefile文件及输出的make "V="命令
|bin/ucore.img
| 生成ucore.img的相关代码为
...
根据lab1/tools/sign.c，定义的一个buf大小为512,说明一个硬盘主引导扇区的大小应为512字节。且第510字节为0x55，第511字节为0xAA;
...
```
开启A20：通过将键盘控制器上的A20线置于高电位，全部32条地址线可用，代码如下：
```
 # Enable A20:
...
【练习五】请完成实验，看看输出是否与上述显示大致一致，并解释最后一行各个数值的含义。
最后一行的输出结果为
```
ebp:0x00007bf8 eip:0x00007d68 args:s0xc031fcfa 0xc08ed88e 0x64e4d08e 0xfa7502a8 
    <unknow>: -- 0x00007d67 --
```
...
【练习六】中断描述符表（也可简称为保护模式下的中断向量表）中一个表项占多少字节？其中哪几位代表中断处理代码的入口？
